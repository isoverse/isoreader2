# Authors and Citation

## Authors

- **Sebastian Kopf**. Author, maintainer.
  [](https://orcid.org/0000-0002-2044-0201)

## Citation

Source:
[`inst/CITATION`](https://github.com/isoverse/isoreader2/blob/v0.6.1/inst/CITATION)

Kopf, Sebastian, Davidheiser-Kroll, Brett, Kocken, Ilja (2021).
“Isoreader: An R package to read stable isotope data files for
reproducible research.” *Journal of Open Source Software*, **6**(61),
2878. [doi:10.21105/joss.02878](https://doi.org/10.21105/joss.02878).

    @Article{,
      title = {Isoreader: An R package to read stable isotope data files for reproducible research},
      author = {{Kopf} and {Sebastian} and {Davidheiser-Kroll} and {Brett} and {Kocken} and {Ilja}},
      journal = {Journal of Open Source Software},
      year = {2021},
      volume = {6},
      number = {61},
      pages = {2878},
      doi = {10.21105/joss.02878},
    }
