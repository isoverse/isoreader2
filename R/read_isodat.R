...
.get_gas_names <- function(json_path, ext, hw_list) {
  ...
  if (!is.data.frame(hw_list)) return(top_gas)   # single-gas

  # multi-gas: append gas name from each sub-method in order
  subs <- RcppSimdJson::fload(json_path, query = .sub_methods_ptr[[gas_ext]], query_error_ok = TRUE)
...
read_isodat_resistors <- function(json_path) {
  ext <- tolower(tools::file_ext(sub("\\.json$", "", json_path, ignore.case = TRUE)))

  if (ext == "scn") return(NULL)
  ...
  hw_list <- switch(ext,
    ...
    did = {
      x <- RcppSimdJson::fload(json_path, query = .hw_list_ptr$did_numeric, query_error_ok = TRUE)
      if (is.null(x)) RcppSimdJson::fload(json_path, query = .hw_list_ptr$did_direct) else x
    },
