...
intensity_unit_info <- function(unit) {
  if (unit == "cps") return(list(base = "cps", scale = 1))
  list(
    base = substr(unit, nchar(unit), nchar(unit)),
    scale = switch(
      unit,
      "mV" = 1e3,  "V" = 1,
      "fA" = 1e15, "pA" = 1e12, "nA" = 1e9, "µA" = 1e6, "mA" = 1e3, "A" = 1
    )
  )
}
...
ir_convert_intensity <- function(
  ...
  if (needs_R) {
    if (is.null(resistors)) {
      cli_abort(
        "converting {.field {src_col}} to {.val {units}} requires resistor values but {.arg resistors} is NULL"
      )
    }
    resistors |> check_tibble(req_cols = c("species", "channel", "resistance.Ohm"))
    ...
    dataset <- dataset |>
      dplyr::left_join(
        resistors |> dplyr::select(dplyr::all_of(c(join_keys, "resistance.Ohm"))),
        by = join_keys
      )
  }
  ...
  intensity_tgt_base <- switch(paste0(src$base, "_", tgt$base),
