...
#' `"29/28"`) and `ratio` (the intensity of that mass divided by the intensity of
#' the base mass of the same species at the same `time.s`/`cycle`/`x` position
#' within every `uidx` and `analysis`). Base mass rows are kept and have `NA` in
#' both columns. Calling this function again recomputes (overwrites) the
...
ir_calculate_ratios <- function(aggregated_data, ...) {
  ...
  # add ratio columns to each present series
...
# share cycle numbers). Base mass rows (and species with an unusable requested
# base mass) get NA in both columns; all rows are retained. Any pre-existing
# `ratio_name`/`ratio` columns are dropped first so the call is idempotent.
# Returns a list with `data` (the series with the two columns) and `base` (the
# named character vector of base masses chosen per species).
add_series_ratios <- function(
  ...
  # ratio = intensity(mass) / intensity(base mass); NA for the base mass itself
  # (and where no base mass could be determined). All rows are kept.
  data <- data |>
