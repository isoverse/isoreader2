...
#' # Additive offsets
#'
#' Which pair of additive offsets is used depends on the intensity unit family of
#' the data: voltage (`V`, `mV`) uses `num_add.V`/`denom_add.V`, current (`A`,
#' `mA`, `µA`, `nA`, `pA`, `fA`) uses `num_add.nA`/`denom_add.nA`, and counts
#' (`cps`) uses `num_add.cps`/`denom_add.cps`. The offsets are specified in their
#' family's reference unit (volts, nanoamperes, cps) and are automatically scaled
#' to the data's actual intensity unit before being added. For example, with
...
#'   denominator intensities of voltage-unit data (`V`/`mV`). Default
...
#'   (`A`/`mA`/`µA`/`nA`/`pA`/`fA`). Default `num_add.nA = 0`,
#'   `denom_add.nA = num_add.nA`.
#' @param num_add.cps,denom_add.cps additive offset (in cps) for the numerator and
#'   denominator intensities of count-unit data (`cps`). Default
#'   `num_add.cps = 0`, `denom_add.cps = num_add.cps`.
