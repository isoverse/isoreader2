...
✔ [2s] check_assembly() successfully installed isoextract version 0.2.1.0
...
``` fansi
✔ [594ms] ir_aggregate_isofiles() aggregated metadata (2), traces (24.5k,
intensity in mV), resistors (8), and vendor_data_table (25) from 2 files using
the extended aggregator
```
...
``` fansi
✔ [151ms] ir_extract_isofiles() finished extracting 2 files/archives
```

``` fansi
✔ [522ms] ir_read_isofiles() finished reading 2 isotope data files/archives
```

``` fansi
✔ [265ms] ir_aggregate_isofiles() aggregated metadata (2) and cycles (192,
...
``` fansi
✔ [271ms] ir_extract_isofiles() finished extracting 4 files/archives
```
...
``` fansi
✔ [956ms] ir_export_to_excel() exported 2 rows of metadata and 24.5k rows of
