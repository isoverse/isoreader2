...
✔ [1.8s] check_assembly() successfully installed isoextract version 0.3.0.0

→ Trying to install isosolfs (helper to open Qtegra notebooks) for your
  operating system isosolfs-linux-x64 (this requires an internet connection and
  may take a moment)...
✔ [2.3s] check_assembly() successfully installed isosolfs (helper to open
Qtegra notebooks) version 0.9.0.0

✔ [118ms] ir_extract_isofiles() finished extracting 2 files/archives

✔ [270ms] ir_read_isofiles() finished reading 2 isotope data files/archives
```
...
``` fansi
✔ [54ms] ir_save_isofiles() saved 2 isofiles to tmp/my_isofiles.rds
```
...
``` fansi
✔ [4ms] ir_load_isofiles() loaded 2 isofiles from tmp/my_isofiles.rds
```
...
``` fansi
✔ [134ms] ir_aggregate_isofiles() aggregated metadata (2) and traces (24.5k,
...
``` fansi
✔ [308ms] ir_aggregate_isofiles() aggregated metadata (2), traces (24.5k,
...
``` fansi
✔ [90ms] ir_aggregate_isofiles() aggregated metadata (2) and traces (24.5k,
...
``` fansi
✔ [6ms] ir_get_data() retrieved 24.5k records from the combination of metadata
...
``` fansi
✔ [313ms] ir_aggregate_isofiles() aggregated metadata (2), traces (24.5k,
intensity in mV), resistors (8), and vendor_data_table (25) from 2 files using
the extended aggregator
```
...
``` fansi
✔ [5ms] ir_get_data() retrieved 25 records from the combination of metadata (2)
...
``` fansi
✔ [93ms] ir_extract_isofiles() finished extracting 1 file/archive
```
...
``` fansi
✔ [81ms] ir_aggregate_isofiles() aggregated metadata (1) and cycles (102,
...
``` fansi
✔ [83ms] ir_extract_isofiles() finished extracting 4 files/archives
```

``` fansi
✔ [255ms] ir_read_isofiles() finished reading 4 isotope data files/archives
```

``` fansi
✔ [190ms] ir_aggregate_isofiles() aggregated metadata (4) and scans (17.8k,
...
``` fansi
✔ [5ms] ir_get_data() retrieved 24.5k records from the combination of metadata
(2) and traces (24.5k) via uidx and analysis
```

``` fansi
✔ [562ms] ir_export_to_excel() exported 2 rows of metadata and 24.5k rows of
