...
``` fansi
✔ [433ms] ir_read_isofiles() finished reading 2 isotope data files/archives
```
...
``` fansi
✔ [250ms] ir_aggregate_isofiles() aggregated metadata (2) and traces (24.5k,
...
``` fansi
✔ [536ms] ir_aggregate_isofiles() aggregated metadata (2), traces (24.5k,
...
``` fansi
✔ [145ms] ir_aggregate_isofiles() aggregated metadata (2) and traces (24.5k,
...
``` fansi
✔ [535ms] ir_aggregate_isofiles() aggregated metadata (2), traces (24.5k,
intensity in mV), resistors (8), and vendor_data_table (25) from 2 files using
the extended aggregator
```
...
``` fansi
✔ [12ms] ir_get_data() retrieved 25 records from the combination of metadata
...
``` fansi
✔ [26ms] ir_filter_masses() kept masses 44, 45, and 46 and removed 20.5k of
...
``` fansi
✔ [22ms] ir_calculate_ratios() calculated 13.5k ratios with additive offsets
...
``` fansi
✔ [27ms] ir_calculate_ratios() calculated 13.5k ratios with additive offsets
num_add.V = 100 and denom_add.V = 100 normalized by `median()` and added
ratio_name and ratio columns to the traces
```
...
``` fansi
✔ [369ms] ir_read_isofiles() finished reading 1 isotope data file/archive
```

``` fansi
✔ [122ms] ir_aggregate_isofiles() aggregated metadata (1) and cycles (102,
intensity in V) from 1 file using the standard aggregator
```

``` fansi
✔ [9ms] ir_calculate_ratios() calculated 85 ratios and added ratio_name and
...
``` fansi
✔ [158ms] ir_read_isofiles() finished reading 4 isotope data files/archives
```

``` fansi
✔ [327ms] ir_aggregate_isofiles() aggregated metadata (4) and scans (17.8k,
...
``` fansi
✔ [952ms] ir_export_to_excel() exported 2 rows of metadata and 24.5k rows of
