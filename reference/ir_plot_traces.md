...
`ir_plot_traces()` - it takes the same arguments and returns the same
plot. Use `ir_plot_traces()` instead, it is the more flexible name for
what the function actually plots (trace data, which continuous flow
files contain).
...
- ...:
  ...
  or
  [`ggplot2::facet_grid()`](https://ggplot2.tidyverse.org/reference/facet_grid.html)
  (e.g. `labeller`); for the deprecated `ir_plot_continuous_flow()` all
  arguments are simply passed on to `ir_plot_traces()`
