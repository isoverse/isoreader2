...
test_that("ir_convert_intensity()", {
  r <- tibble(species = "CO2", channel = 1L, resistance.Ohm = R)
  d_V <- tibble(species = "CO2", channel = 1L, intensity.V = 3.0)
  d_mV <- tibble(species = "CO2", channel = 1L, intensity.mV = 3000.0)
  d_A <- tibble(species = "CO2", channel = 1L, intensity.A = 1e-11)
  d_nA <- tibble(species = "CO2", channel = 1L, intensity.nA = 10.0)
  d_cps <- tibble(species = "CO2", channel = 1L, intensity.cps = 1e6)

  # argument errors ============================================================
  expect_error(ir_convert_intensity(list()), "data frame")
  expect_error(ir_convert_intensity(tibble(species = "CO2")), "channel")
  expect_error(
    ir_convert_intensity(tibble(species = "CO2", channel = 1L)),
    "intensity"
  )
  expect_error(ir_convert_intensity(d_V, units = "xxx"), "units")
  # resistors required for V <-> A/cps conversions
  expect_error(ir_convert_intensity(d_V, units = "nA"), "requires resistor")
  expect_error(ir_convert_intensity(d_V, units = "cps"), "requires resistor")
  expect_error(ir_convert_intensity(d_mV, units = "pA"), "requires resistor")
  expect_error(ir_convert_intensity(d_A, units = "mV"), "requires resistor")
  expect_error(ir_convert_intensity(d_cps, units = "V"), "requires resistor")
  ...
  expect_equal(
    ir_convert_intensity(d_A, units = "nA")$intensity.nA,
    1e-11 * 1e9
  )
  expect_equal(
    ir_convert_intensity(d_A, units = "pA")$intensity.pA,
    1e-11 * 1e12
  )
  expect_equal(
    ir_convert_intensity(d_A, units = "fA")$intensity.fA,
    1e-11 * 1e15
  )
  ...
  expect_equal(
    ir_convert_intensity(d_nA, units = "fA")$intensity.fA,
    10 * 1e-9 * 1e15
  )

  # current <-> cps (no resistors needed) =====================================
  expect_equal(
    ir_convert_intensity(d_A, units = "cps")$intensity.cps,
    1e-11 / e
  )
  expect_equal(ir_convert_intensity(d_cps, units = "A")$intensity.A, 1e6 * e)
  expect_equal(
    ir_convert_intensity(d_cps, units = "fA")$intensity.fA,
    1e6 * e * 1e15
  )
  ...
  expect_equal(
    ir_convert_intensity(d_V, r, units = "nA")$intensity.nA,
    3.0 / R * 1e9
  )
  expect_equal(
    ir_convert_intensity(d_V, r, units = "fA")$intensity.fA,
    3.0 / R * 1e15
  )
  expect_equal(ir_convert_intensity(d_A, r, units = "V")$intensity.V, 1e-11 * R)
  expect_equal(
    ir_convert_intensity(d_A, r, units = "mV")$intensity.mV,
    1e-11 * R * 1e3
  )
  expect_equal(
    ir_convert_intensity(d_mV, r, units = "pA")$intensity.pA,
    3.0 / R * 1e12
  )

  # voltage <-> cps (resistors required) ======================================
  expect_equal(
    ir_convert_intensity(d_V, r, units = "cps")$intensity.cps,
    3.0 / R / e
  )
  expect_equal(
    ir_convert_intensity(d_cps, r, units = "V")$intensity.V,
    1e6 * e * R
  )
  expect_equal(
    ir_convert_intensity(d_cps, r, units = "mV")$intensity.mV,
    1e6 * e * R * 1e3
  )
  ...
  r_no_cfg <- tibble(species = "CO2", channel = 1L, resistance.Ohm = R)
  expect_equal(
